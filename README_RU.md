...
### 1. Установка
1. Скачайте плагин и поместите папку `android_IAPP` в `res://addons/`.
...
### ⚙️ Настройка экспорта Android (Критически важно!)

Чтобы избежать ошибки Google Play Console *"Ваше приложение использует версию AIDL библиотеки Google Play Billing..."*, настройте профиль экспорта Android (`Проект -> Экспорт -> Android`):

1. **Включите сборку Gradle (Custom Build)**: В разделе `Параметры` установите галочку **Использовать сборку Gradle**. Это позволяет Gradle автоматически скачать и включить библиотеку `com.android.billingclient:billing-ktx` в ваш APK/AAB.
2. **Отключите встроенный биллинг Godot**: В разделе `Плагины` **СНИМИТЕ** галочку со встроенного плагина **Google Play Billing**, так как он содержит устаревший код AIDL.
3. **Разрешения**: В разделе `Разрешения` убедитесь, что включено разрешение **Billing**.
...
**Совместимость:** Godot 4.3+ (Android Export c включенной сборкой Gradle). Требуется отключенный встроенный плагин Google Play Billing и включенное разрешение `Billing` в настройках экспорта.
